...
{
    BoardLCD_FastDrawPoint((uint16_t)x,(uint16_t)y,(uint16_t)PixelIndex);
}
...
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  unsigned int PixelIndex;
    ...
    GUI_USE_PARA(y);
    {
      ...
      // TBD by customer...
      //
      PixelIndex = BoardLCD_ReadPoint((uint16_t)x,(uint16_t)y);
    }
...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) 
{
    ...
    BoardLCD_FillSingleColor((uint16_t)x0,(uint16_t)y0,(uint16_t)x1,(uint16_t)y1,(uint16_t)PixelIndex);
}
